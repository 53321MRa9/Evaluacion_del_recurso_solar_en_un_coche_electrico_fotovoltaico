...
def _get_irradiance(site_location, date, periodo, tilt, surface_azimuth,
    ...
    # El método get_clearsky devuelve una estructura de datos con los
    ...
    # Asignamos los valores de irradiancia difusa donde se producen sombras
    # Si los puntos no producen sombra, les asignamos la irradiancia global
    for i in range(len(clearsky_sombra)):
        if clearsky_sombra.index[i] not in puntos:
            GHI_all.append(clearsky_sombra['dhi'][i])
        else:
            GHI.append(clearsky_sombra['ghi'][i])
            GHI_all.append(clearsky_sombra['ghi'][i])

    for i in range(len(POA_irradiance)):
        if POA_irradiance_sombra.index[i] not in puntos:
            POA_all.append(POA_irradiance_sombra['poa_diffuse'][i])
        else:
            POA.append(POA_irradiance_sombra['poa_global'][i])
            POA_all.append(POA_irradiance_sombra['poa_global'][i])
...
def _get_irradiance_day(site_location, date, frec, tilt, surface_azimuth,
    ...
    for i in range(len(clearsky_sombra)):
        if clearsky_sombra.index[i] not in puntos:
            clearsky_sombra['ghi'][i] = clearsky_sombra['dhi'][i]

    for i in range(len(POA_irradiance)):
        if POA_irradiance_sombra.index[i] not in puntos:
            POA_irradiance_sombra['poa_global'][i] = POA_irradiance_sombra['poa_diffuse'][i]
